...
HyperCube(left::Vector{T}, right::Vector{T}) where T = HyperCube{T, length(left)}(left, right)
HyperCube(::Type{T}, d::Int) where T = HyperCube(-ones(T, d), ones(T, d))
HyperCube(d::Int) = HyperCube(Float64, d)

center(hypercube::HyperCube) = 0.5*(hypercube.left + hypercube.right)
Base.rand(hypercube::HyperCube{T,d}) where {T,d} = (hypercube.right - hypercube.left).*rand(T,d) + hypercube.left
