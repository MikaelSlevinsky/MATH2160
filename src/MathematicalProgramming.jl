...
function contourf(problem::Problem{2},N=301,L=50;cmap="seismic")
    ...
    x = range(xl, stop = xr, length = N)
    y = range(yl, stop = yr, length = N)
...
function makegif(solution::Solution{2})
    ...
    x = range(xl, stop = xr, length = 101)
    y = range(yl, stop = yr, length = 101)
    xx, yy = [x for y in y, x in x], [y for y in y, x in x]
    fplotvals = map(f,map(tuple,xx,yy))
...
surf(problem1,501)
savefig(math2160*"/MathematicalProgrammingf1.png";dpi=600)

problem2 = Problem(f, HyperCube(2))

contourf(problem2)
savefig(math2160*"/MathematicalProgrammingf2.png";dpi=600)
...
for i = 10:10:180
    local solution = prokaryoticsolve(problem; nmembers = i, withprint = false)
    println("i = ", i,", and the fitness is: ", solution.minimum)
end

for i = 10:10:180
    local solution = eukaryoticsolve(problem; nmembers = i, withprint = false)
    println("i = ", i,", and the fitness is: ", solution.minimum)
end
